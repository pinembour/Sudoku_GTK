/*Universidade da Beira Interior - Tecnologias e Sistemas de Informacao
  ...
  -A chamada dos subprogramas 'apresentaOK' e 'apresentaProblema' nao foi implementada e encontra-se dentro de comentarios para o programa correr na normalidade. Enquanto isso na consola/terminal sao apresentado as mensagens de informacao.*/
...
void apresentaOK(GtkWidget *widget, gpointer window){
  GtkWidget *dialog;
  ...
  dialog = gtk_message_dialog_new(window,GTK_DIALOG_DESTROY_WITH_PARENT,
	...
	"Sudoku com valores introduzidos correctamente!VENCEU!", "title");
...
void apresentaProblema(GtkWidget *widget, gpointer window){
  GtkWidget *dialog;
  
  //Texto a apresentar na janela, ao lado do icon de warning
  dialog = gtk_message_dialog_new(window,GTK_DIALOG_DESTROY_WITH_PARENT,
	GTK_MESSAGE_WARNING,GTK_BUTTONS_OK,
	"Contem valores introduzidos incorrectamente.");
...
//		g_signal_connect(G_OBJECT(check), "activated",G_CALLBACK(apresentaOK), NULL);
	}
	else{
		printf("Apresenta a janela de warning\n"); //printf apenas para testes
//		g_signal_connect(G_OBJECT(check), "activated",G_CALLBACK(apresentaProblema), NULL);
	}
}
